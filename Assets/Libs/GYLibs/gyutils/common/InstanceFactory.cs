...
/// <summary>
/// benifit over 100,000 calls. Better than Activator.CreateIns
/// </summary>
public static class InstanceFactory
{
    ...
    {
        return InstanceFactoryGeneric<TypeToIgnore, TypeToIgnore, TypeToIgnore>.CreateEmptyInstance(type);
    }
...
public static class InstanceFactoryGeneric<TArg1, TArg2, TArg3>
{
    ...
    private static ConcurrentDictionary<Type, Func<object>> cachedEmptyFuncs = new ConcurrentDictionary<Type, Func<object>>();
    ...
    public static object CreateEmptyInstance(Type type)
    {
        if (cachedEmptyFuncs.TryGetValue(type, out Func<object> func))
            return func();
        else
            return CacheEmptyFunc(type)();
    }
    ...
    private static Func<object> CacheEmptyFunc(Type type)
    {
        var constructorTypes = new List<Type>();
        ...
        var func = lambdaExpr.Compile();
        cachedEmptyFuncs.TryAdd(type, func);
        return func;
    }
}
...
private void TestPerformance()
{
    ...
    System.Type type = typeof(Guest.fsm.Common.StateDelay);
    sw.Start();

    var constructor = System.LambdaActivator.GetMatchingConstructor(type);
    var deleFunc = System.LambdaActivator.GetActivator(constructor);

    for (int i = 0; i < times; i++)
    {
        Guest.fsm.BaseGuestState state = deleFunc() as Guest.fsm.BaseGuestState;
    }
    sw.Stop();

    Debug.Log("lamb sw cost : " + sw.ElapsedTicks);
    sw = new System.Diagnostics.Stopwatch();
    sw.Start();
    for (int i = 0; i < times; i++)
    {
        Guest.fsm.BaseGuestState state = System.Activator.CreateInstance(type) as Guest.fsm.BaseGuestState;
    }
    sw.Stop();
    Debug.Log("act sw cost : " + sw.ElapsedTicks);

    sw = new System.Diagnostics.Stopwatch();
    sw.Start();
    for (int i = 0; i < times; i++)
    {
        Guest.fsm.BaseGuestState state = InstanceFactory.CreateInstance(type) as Guest.fsm.BaseGuestState;
    }
    sw.Stop();
    Debug.Log("ins sw cost : " + sw.ElapsedTicks);
}
 */
